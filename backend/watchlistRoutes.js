...
router.post('/', async (req, res) => {
  try {
    ...
    const db = getDB();
    await db.run(
      `INSERT INTO watchlist_items (stock_symbol) VALUES (?)`,
      [symbol.toUpperCase()]
    );
...
router.delete('/:symbol', async (req, res) => {
  try {
    ...
    const result = await db.run(
      `DELETE FROM watchlist_items WHERE stock_symbol = ?`,
      [symbol]
    );
...
router.get('/changes', async (req, res) => {
  try {
    ...
    const stocks = await db.all(`
      SELECT
        stock_symbol, price, change_percentage,
        anomaly_level, volume, fetched_at
      FROM price_history
      WHERE id IN (
        SELECT MAX(id) FROM price_history GROUP BY stock_symbol
      )
    ...
    const changes = stocks.map(stock => {
      ...
      return {
        ...
        attention:
          Math.abs(change) >= 5 ? "HIGH" :
          Math.abs(change) >= 2 ? "MEDIUM" : "LOW",
        message:
          Math.abs(change) >= 5 ? `${stock.stock_symbol} moved significantly since your last check` :
          Math.abs(change) >= 2 ? `${stock.stock_symbol} showed a noticeable movement` :
